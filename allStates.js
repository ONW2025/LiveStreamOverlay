...
function confirmStateSelection() {
  console.log("➡️ confirmStateSelection called");
  const stateSelector = document.getElementById("stateSelector");
  const selected = Array.from(stateSelector.selectedOptions).map(
    (opt) => opt.value
  );
  ...
  document.getElementById("stateSelectorWrapper").style.display = "none";
  lastSidebarSnapshot = ""; // Force sidebar to update after state change
  fetchAndDisplayAlerts(); // First fetch
  setTimeout(() => { // Introduce a slight delay before starting the interval
    setInterval(fetchAndDisplayAlerts, 6000); // Set refresh loop
  }, 100); // A short delay to ensure the first fetch completes
}
...
function formatCountyList(areaDesc) {
  ...
  return `${counties.slice(0, -1).join(", ")}, and ${counties.slice(
    -1
  )} Counties`;
}
...
async function fetchAndDisplayAlerts() {
  try {
    ...
    const stateQuery = selectedStates.join(",");
    const res = await fetch(`https://api.weather.gov/alerts/active.atom?area=${stateQuery}`);
    ...
    entries.forEach((entry) => {
      ...
      const expiresText = expiresDate.toLocaleTimeString("en-US", {
      ...
      const baseText = `${event} – ${areaDesc} until ${expiresText}`;
      const alertObj = {
        id,
        text: baseText + hazardsText,
        type: eventColor(event),
        expires: expiresDate.toISOString() // use real ISO string for timing
      };
      ...
      if (!seenAlertIds.has(id)) {
        if (sent > pageLoadTime && msgType === "Alert") {
          triggerPopup(`🚨 ${event} for ${areaDesc} until ${expiresText}`, event);
        }
        seenAlertIds.add(id);
      }
    });
    ...
    console.log("➡️ Calling updateSidebar"); // Add this line
...
async function fetchStormReports() {
  try {
    const res = await fetch(
      "https://mesonet.agron.iastate.edu/geojson/lsr.geojson?states=OH&hours=2"
    );
    ...
    reports.forEach((report) => {
      ...
      const time = new Date(props.valid).toLocaleTimeString("en-US", {
        timeZone: "America/New_York",
      });
...
function addStormReport(text) {
  ...
  if (chime) {
    ...
    chime.play().catch((err) => {
      console.warn(
        "🔇 Could not play LSR chime (possibly blocked by browser autoplay policy):",
        err
      );
    });
  }
